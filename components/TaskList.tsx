...
function formatWhen(when: string): string {
  // Prettify ISO dates; pass through keywords as-is
...
export default function TaskList({ tasks, onTasksChange, onReset }: TaskListProps) {
  ...
  const commitEdit = (index: number) => {
    if (editValue.trim()) {
      onTasksChange(
        tasks.map((t, i) => (i === index ? { ...t, title: editValue.trim() } : t))
      );
    }
  ...
  const addToThings = () => {
    const data = tasks.map(({ title, when }) => ({
      type: "to-do",
      attributes: {
        title,
        ...(when ? { when } : {}),
    ...
    <div className="flex flex-col gap-4">
      ...
      <ul className="flex flex-col gap-2">
        {tasks.map((task, i) => (
          <li
            key={i}
            className="flex flex-col bg-gray-800 rounded-2xl px-4 py-3 gap-1"
          >
          ...
          <input
            ...
            onBlur={(e) => {
              if (e.target.value.trim()) {
                onTasksChange([...tasks, { title: e.target.value.trim() }]);
              }
              setAdding(false);
            }}
            onKeyDown={(e) => {
